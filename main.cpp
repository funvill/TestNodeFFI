...
typedef uint32_t (*FPExample)(char* buffer, uint32_t maxBufferLength, uint8_t* randomNumber);

FPExample g_example; // global function pointer.

DllExport void RegisterExample(FPExample example)
{
    g_example = example;
}
...
DllExport void Loop()
{
    ...
    std::cout << "CFYI: randomNumber before: " << std::dec << (int)randomNumber << std::endl;

    std::cout << "CFYI: Calling callback" << std::endl;
    // Calling callback
    uint32_t ret = g_example(buffer, BUFFER_SIZE, &randomNumber);
    if (ret > BUFFER_SIZE) {
    ...
    std::cout << "CFYI: randomNumber afterwards: " << std::dec << (int)randomNumber << std::endl;
}
